...
All browser tabs (and the EFB view) share one **persistent** Ultralight
`Session` — `UltralightHost` sets `Config::cache_path` to `<plugin>/cache` and
`CreateSession(true, "xefb")`, so cookies / localStorage survive restarts and a
Navigraph / VATSIM login in one tab is seen by the others. `window.open()` /
`target="_blank"` OAuth popups are handled by `BrowserWindow::OnCreateChildView`
(they open as a normal tab in the same session instead of being dropped).
